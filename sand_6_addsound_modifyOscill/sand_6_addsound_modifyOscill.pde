...
void setup() {
  ...
  // 初始化计时器的时长
  oscillTimer = new lifeStateTimer(1.5);
...
void draw() {
...

  {
    ...
    float oscillBase = 0.05 + 0.15 *(1-expandTimer.state);
    ...
    for ( int i = 0; i < others.length; i++ ) {
      ...
      float at = getTwistValue(x[i], y[i], millis()*0.00040);
      //float at = getTwistValue(x[i], y[i], millis()*0.40);
      ...
      /* 
       画面的变化速度跟twistRatio的变化速度有关，
       twistRatio 的变化速度则跟timeCnter的变化速度有关
       所以控制timeCnter变化的快慢就可以了
       而且用这个机制的话，每次点击完都会是不同的圆圈间距
       */
      float dt = millis() - prevTime;
      if (!oscillTimer.isDead()) {
        //timeCnter += dt *0.001 * 1.6 ; // 点击鼠标时的变化速度

        // 因为观察到貌似是timeCnter跨越某个区间的时候震荡会很明显
        // 所以用这个算法，意思是在oscillTimer活着的时候走过TWO_PI的长度
        // 可能不一定是TWO_PI，选这个数字只是因为刚好一个周期
        timeCnter += TWO_PI / (oscillTimer.lifeSpan * frameRate);
      } else {
        timeCnter += dt*0.0001 * 2.9; // 无点击时的变化速度
      }
      prevTime = millis();
      ...
      //float twistRatio = map(sin(millis()*0.000001), -1, 1, 0, 1);

      float twistRatio = map(sin(timeCnter), -1, 1, 0, 1);
      ...
      float f = sin(dfac * d *
        (oscillBase + oscillAmp * sin(0*TWO_PI*oscillTimer.state) )) * m[i];
      f /= (d*1.0);

      float iterateRatio = 0.73; // 原速度的保留比例

      vx[i] = vx[i] * iterateRatio + f * dx ; // dx+, vx +
      vy[i] = vy[i] * iterateRatio + f * dy;

      x[i] += vx[i];
      y[i] += vy[i];

      //if (x[i] < 0) x[i] = random(width);
      //else if (x[i] > width) x[i] = random(width);

      //if (y[i] < 0) y[i] = random(height);
      //else if (y[i] > height) y[i]  = random(height);
      // 改变位置重置的方式也会有影响
      ...
      // 屏幕上有点看不太清楚，稍微调高了 strokeWeight 和透明度
      if (rand[i%rand.length]>0.118) {
        strokeWeight(1.1);
        stroke(145, 201, 248, 165);
      } else if (rand[i%rand.length]>0.113) {
        strokeWeight(3.9);
        stroke(254, 235, 199, 173);
      } else {
...
float getTwistValue(float x, float y, float seed) {
  ...
  return unknown_factor* 1.0*map(noise(seed), 0, 1, 1, 1.2);
