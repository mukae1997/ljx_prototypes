...
static public class MyParticleSystem {
  ...
  float particle_color_h = 0.7f, particle_color_s=0.6f, 
  particle_color_b=0.6f;
  ...
  public void resize(DwPixelFlow context, int num_particels_x, int num_particels_y) {
    ...
    // ------------- my modify -------------
    int REDUCE_FACTOR = 1; // MAX_PARTICLES will be divided by this number
    // ------------- my modify -------------
  ...
  public void render(PGraphics2D dst, int background) {
    ...
    if (background == 0) 
      dst.blendMode(PConstants.ADD); // works nicely on black background
    ...
    //shader_particleRender.uniform1f     ("pch", 
    //  particle_color_h);
    //shader_particleRender.uniform1f     ("hval", 
    //  particle_color_h);
