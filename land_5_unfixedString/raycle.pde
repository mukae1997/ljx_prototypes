...
class rayParticle extends Particle3D {
  ...
  void update() {
    ...
    // ----------------------------
    history.add(0, pos.copy());
    int maxsize = 19; // 粒子拖尾长度
    if (history.size() > maxsize) {
      history.remove(floor(maxsize));
    }
    // ----------------------------
    angle += angleRate*0.25; // 转速，越大，越快形成漩涡
  }
  void show() {
    pushMatrix();
    ...
    for (int i = 0; i < history.size(); i++ ) { 
      ...
      float maxAlpha = 61; // 头部粒子位置的透明度
      stroke(255, map(state, 0, 1, map(i, 0, history.size(), maxAlpha, 16), 5));
